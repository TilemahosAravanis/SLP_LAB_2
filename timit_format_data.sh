...
# data/local/, and converts it into the "canonical" form,
...
for x in train dev test; do
  mkdir -p data/$x
  cp $srcdir/$x/${x}_wav.scp data/$x/wav.scp || exit 1;
  cp $srcdir/$x/${x}_text.txt data/$x/text || exit 1;
  cp $srcdir/$x/${x}_spk2utt.txt data/$x/spk2utt || exit 1;
  cp $srcdir/$x/${x}_utt2spk.txt data/$x/utt2spk || exit 1;
